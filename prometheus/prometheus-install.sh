...
sed -i 's/{KUBE_RBAC_PROXY_VERSION}/'${KUBE_RBAC_PROXY_VERSION}'/g' $SCRIPTDIR/yaml/manifests/node-exporter-daemonset.yaml
sed -i 's/{PROMETHEUS_ADAPTER_VERSION}/'${PROMETHEUS_ADAPTER_VERSION}'/g' $SCRIPTDIR/yaml/manifests/prometheus-adapter-deployment.yaml
sed -i 's/{PROMETHEUS_VERSION}/'${PROMETHEUS_VERSION}'/g' $SCRIPTDIR/yaml/manifests/prometheus-prometheus.yaml
sed -i 's/{PROMETHEUS_OPERATOR_VERSION}/'${PROMETHEUS_OPERATOR_VERSION}'/g' $SCRIPTDIR/yaml/setup/prometheus-operator-deployment.yaml
sed -i 's/{CONFIGMAP_RELOADER_VERSION}/'${CONFIGMAP_RELOADER_VERSION}'/g' $SCRIPTDIR/yaml/setup/prometheus-operator-deployment.yaml
sed -i 's/{CONFIGMAP_RELOAD_VERSION}/'${CONFIGMAP_RELOAD_VERSION}'/g' $SCRIPTDIR/yaml/setup/prometheus-operator-deployment.yaml

## step1: apply setup
kubectl create -f $SCRIPTDIR/yaml/setup/
sleep 30s

## step2: apply manifests
kubectl create -f $SCRIPTDIR/yaml/manifests/
...
kubectl get prometheus k8s -n monitoring -o json | jq 'del(.spec.storage)' > $SCRIPTDIR/prometheus-k8s.json
kubectl delete prometheus k8s -n monitoring
kubectl apply -f prometheus-k8s.json

rm $SCRIPTDIR/prometheus-k8s.json -f

## step3: apply discovery
kubectl create -f $SCRIPTDIR/yaml/kube-controller-manager-prometheus-discovery.yaml
kubectl create -f $SCRIPTDIR/yaml/kube-scheduler-prometheus-discovery.yaml

kubectl get servicemonitor -n monitoring kube-controller-manager -o json | jq 'del(.spec.endpoints[].metricRelabelings)' > $SCRIPTDIR/kube-controller-manager.json
kubectl get servicemonitor -n monitoring kube-scheduler -o json | jq 'del(.spec.endpoints[].metricRelabelings)' > $SCRIPTDIR/kube-scheduler.json

kubectl apply -f $SCRIPTDIR/kube-controller-manager.json
kubectl apply -f $SCRIPTDIR/kube-scheduler.json

rm $SCRIPTDIR/kube-controller-manager.json  -f
